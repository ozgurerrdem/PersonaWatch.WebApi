...
public class YouTubeScannerService : IScanner
{
    ...
    public async Task<List<NewsContent>> ScanAsync(string personName)
    {
        ...
        var searchUrl = $"https://www.googleapis.com/youtube/v3/search?part=snippet&q={Uri.EscapeDataString(personName)}&type=video&maxResults=50&order=date&publishedAfter={publishedAfter}&key={_apiKey}";
